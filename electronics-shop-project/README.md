...
- Frontend đã tối ưu tầng gọi API: `lib/config.ts` + `lib/apiClient.ts` + `lib/services/*` dùng chung, tự gắn token, tự báo lỗi kết nối rõ ràng; trang chủ và trang quản trị đã fetch dữ liệu thật từ BE (không còn dữ liệu mẫu cứng); thêm trang `/admin/login` để lấy token thật
- Đã build thử `npm run build` và `tsc --noEmit` thành công, không lỗi

Cần bổ sung tiếp (xem TODO trong `backend/app/api/v1/router.py`):
- Cart, Order, Installment (trả góp), Promotion/Discount, Category CRUD riêng, quản lý tài khoản nhân viên (tạo/sửa)
- Trang đăng nhập nhân viên trên frontend để lưu token thật (hiện đang đọc tạm từ `localStorage`)
